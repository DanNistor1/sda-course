...
public class AppConfig {
    ...
    private void findBookUsingOptionalOrElseThrow() {
        Book savedBook = bookRepository.findById(1L).orElseThrow(
                () -> new RuntimeException("Book not found")); // orElseThrow if a value is present, returns the value, otherwise throws an exception produced by the exception supplying function.
        log.info("saved book: {}", savedBook);
    }

    private void findBookUsingOptionalGet() {
        Optional<Book> savedBookOptional = bookRepository.findById(1L); // bookRepository.findById(1L) returns an Optional
        Book savedBook = null;
